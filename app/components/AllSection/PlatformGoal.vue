<template>
  <section class="my-40! text-primaryTwo dark:text-textsecondary">
    ...
    >
      <!-- Title -->
      <h3
        class="text-center font-semibold text-xl md:text-4xl text-primaryTwo dark:text-secondary"
      >
    ...
    <!-- Main Content Card -->
    <v-container
      class="border rounded-2xl mt-14 shadow-sm backdrop-blur-sm dark:shadow-gray-300/20"
    >
      <div
        ...
        :class="
          lang === 'ar'
            ? 'sm:flex! sm:flex-row-reverse! flex-wrap!'
            : 'sm:flex! flex-wrap!'
        "
      ...
      >
        <!-- Text -->
        <p
          class="leading-[1.9] sm:max-w-[50%] text-[15px] md:text-[18px] text-center text-textmain dark:text-white opacity-80"
        >
          {{ $t("section_platform_goal.paragraph") }}
        </p>
      ...
      <!-- Features Icons -->
      <div
        class="grid grid-cols-3 gap-2 mt-8 text-center text-textmain dark:text-secondary"
        v-gsap.entrance.slide-left.stagger="{
          duration: 1,
          stagger: 0.4,
        }"
      >
        <div v-for="(item, i) in feature" :key="i">
...
<script setup>
// icons
import { mdiBank, mdiTempleBuddhist, mdiSkull } from "@mdi/js";
// Lang
let { lang } = useLang();

// Data Icon Featuer
let feature = [
  { icon: mdiBank, title: "section_platform_goal.feature1" },
  { icon: mdiTempleBuddhist, title: "section_platform_goal.feature2" },
  { icon: mdiSkull, title: "section_platform_goal.feature3" },
];
</script>
